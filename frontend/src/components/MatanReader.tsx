...
const FONT_CLASSES: Record<number, string> = { 1.5: "text-[1.5rem]!", 1.75: "text-[1.75rem]!", 2: "text-[2rem]!", 2.25: "text-[2.25rem]!", 2.5: "text-[2.5rem]!", 2.75: "text-[2.75rem]!", 3: "text-[3rem]!" };

function MatanEntry({ entry, book, fontSize }: Readonly<{ entry: Entry; book: Book; fontSize: number }>) {
  ...
  return <article className="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 sm:p-8 space-y-5">
    ...
    <div><h4 className="text-xs font-semibold text-slate-500 mb-3">Teks Arab dari sumber</h4><p lang="ar" dir="rtl" className={`arabic-text whitespace-pre-wrap wrap-break-word leading-[2.1]! ${FONT_CLASSES[fontSize]}`}>{entry.arabic}</p></div>
    ...
    <output className="block text-sm text-emerald-700 dark:text-emerald-300">{copyStatus}</output>
  </article>;
}
...
export default function MatanReader() {
  ...
  return <div className="space-y-6">
    <fieldset className="flex flex-wrap gap-3">
      <legend className="sr-only">Pilihan kitab</legend>
      {BOOKS.map(book => <label key={book.id} className="cursor-pointer">
        <input type="radio" name="matan-book" value={book.id} checked={bookId === book.id} onChange={() => { setBookId(book.id); setQuery(""); setChapter(""); setPage(1); }} className="peer sr-only" />
    ...
    {loading && <output className="block py-8 text-center text-slate-500">Memuat teks kitab…</output>}
    ...
    {data && <>
      <section className="space-y-3"><h2 className="text-xl font-bold">{data.title} — {data.author}</h2><p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">{data.note}</p><a href={data.source_url} target="_blank" rel="noopener noreferrer" className="inline-block text-sm underline text-emerald-700 dark:text-emerald-300">Sumber: {data.source_name} ↗</a><output className="block text-sm">{data.matched} dari {data.total} teks • Halaman {data.page} / {data.pages}</output></section>
